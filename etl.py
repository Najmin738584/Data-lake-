...
def create_spark_session():
    "creates new or uses existing spark session"
...
def process_song_data(spark, input_data, output_data):
    ...
    song_schema = StructType([
        StructFeild("artist_id", StringType()),
        StructFeild("artist_latitude", DoubleType()),
        StructFeild("artist_location", StringType()),
        StructFeild("artist_longitude", StringType()),
        StructFeild("artist_name", StringType()),
        StructFeild("duration", DoubleType()),
        StructFeild("num_songs", IntegerType()),
        StructFeild("title", StringType()),
        StructFeild("year", IntegerType()),
    ])
    ...
    song_field = ["title", "artist_id", "year", "duration"]
    song_table = df.select(song_field).dropDuplicates().withColumn("song_id", monotonically_increasing_id())
    
    # write songs table to parquet files partitioned by year and artist
    song_table.write.mode("overwrite").partitionBy("year", "artist_id").parquet(output_data + "songs")
...
def process_log_data(spark, input_data, output_data):
    ...
    artists_songs_logs = songs_logs.join(songs_df, (songs_logs.artist == artists_df.name))
...
def main():
    ...
    input_data = "s3a://project4s3bucket/"
    output_data ="s3a://project4s3bucket/output/" 
